...
from BOTs.Binance_Bot_Original.Binance_Orders_Handler import Binance_Orders_Handler
from BOTs.Binance_Bot_Original.debug_webhook_message import DEBUG_WEBHOOK_MESSAGE
...
class Binance_Bot(Application_Handler, Binance_Orders_Handler, Webhook_Handler):
    ...
    def __init__(self, tradingview_bot_function,
        ...
        ** True if getenv("SANDBOX_MODE", default=True) else 'live_test')
        ...
        self.exchange, self.webhook_passphrase = asyncio.run(self.initialize_exchange(env_config))

        # Done
        self.breathing = True
        self.log.info("Initialized Binance Bot 🤖")

    async def initialize_exchange(self, env_config):
        """
    ...
    async def tradingview_webhook_handler(BOT):
