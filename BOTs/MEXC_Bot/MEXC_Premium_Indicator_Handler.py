...
def get_side_of_position_to_enter_or_exit(webhook_signals, position_amount):
    ...
    # fixme  hotfix to prevent hitting the max open orders limit of MEXC
...
def premium_indicator_function(BOT, webhook_message):
    ...
    place the order on the MEXC exchange_client.
    :return:
    """
    ...
    last_price = BOT.exchange_client.index_price(symbol)['data']['indexPrice']
    #
    USDT_free_balance = BOT.exchange_client.asset('USDT')['data']['availableBalance']
    ...
    free_balance_trading_symbol = BOT.exchange_client.asset(symbol.split("_USDT")[0])['data'][
        'availableBalance']  # e.g. BTC or ETH
    current_position_info = BOT.exchange_client.open_positions(symbol=symbol)
    if len(current_position_info["data"]) != 0:
        current_position_amount = float(current_position_info["data"][0]["holdVol"])
    else:
        current_position_amount = float(0)

    # todo can be moved to webhook checks
    if isinstance(BOT.exchange_client, futures.HTTP) and leverage != BOT.exchange_client.get_leverage(symbol):
        BOT.exchange_client.change_leverage(
            position_id=None,
            symbol=symbol,
            leverage=leverage,
            open_type=1,
    ...
    elif isinstance(BOT.exchange_client, spot.HTTP):
